...
export const sellGift = async (req: any, res: Response) => {
  const { userGiftId } = req.body;
  if (!userGiftId) return res.status(400).json({ error: 'Gift is required' });
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!userGift || userGift.receiverId !== req.userId) {
        throw new Error('Only the current owner can sell this gift');
      }

      const baseRefund = Math.max(1, Math.floor(userGift.gift.price * 0.7));
      const nftRefund = userGift.isNft ? Math.floor(NFT_UPGRADE_PRICE * 0.5) : 0;
      const refund = baseRefund + nftRefund;

      await tx.userGift.delete({ where: { id: userGiftId } });

      if (userGift.gift.type === 'LIMITED' && userGift.gift.remaining !== null) {
        await tx.giftTemplate.update({
          where: { id: userGift.giftId },
      ...
      const user = await tx.user.update({
        where: { id: req.userId },
        data: { stars: { increment: refund } },
      });

      return { soldGiftId: userGiftId, refund, stars: user.stars };
    });
